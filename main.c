...
int main() {
    unsigned char *pi_arr = malloc(sizeof(unsigned char) * MAXLEN); // this is 1Bite integer array
    unsigned char *part_goal_str = malloc(sizeof(unsigned char) * MAXLEN); // this is 1Bite integer array
    unsigned int n_size_pi, goal_idx, size_pgs;
    unsigned int ans_idx_arr[SEARCHIDX - 1];
    unsigned int **search_idx_mat;
    unsigned int *base_search_idx_mat;
    unsigned int search_idx_arr[10];
...
unsigned int read_pi(unsigned char *arr) {
    ...
    //for debug
    FILE *fp;
//    if ((fp = fopen("./pi_data/pi200.txt", "r")) == NULL) {
...
unsigned int search(unsigned char *pi_arr, unsigned int n_size_pi, unsigned char *part_goal_str, unsigned int size_pgs,
    ...
    if (size_pgs == 0) {
        ...
        for (i = 0; i < n_search_idxs; ++i) {
            number = pi_arr[search_idx_arr[i] + size_pgs];
    ...
    for (unsigned char j = 0; j < 10; j++) {
        ...
        if (accumu_cnt >= n_remain) {
            if (cnt_arr[j] == 1) {
                ...
                part_goal_str[size_pgs] = j;
                size_pgs++;
                arrcpy(search_idx_arr, search_idx_mat[j], cnt_arr[j]);
                search(pi_arr, n_size_pi, part_goal_str, size_pgs, n_remain - accumu_cnt + cnt_arr[j], search_idx_arr,
                       cnt_arr[j], search_idx_mat);
                break;
            }
//        if (accumu_cnt == n_remain) {
//            return search_idx_mat[j][0];
//        } else if (accumu_cnt > n_remain) {
//            part_goal_str[size_pgs] = j;
//            size_pgs++;
//            arrcpy(search_idx_arr, search_idx_mat[j], size_search_idx_mat[j]);
//            search(pi_arr, n_size_pi, part_goal_str, size_pgs, n_remain - accumu_cnt + cnt_arr[j], search_idx_arr,
//                   size_search_idx_mat[j], search_idx_mat);
//            break;
//        }
        }
    }
